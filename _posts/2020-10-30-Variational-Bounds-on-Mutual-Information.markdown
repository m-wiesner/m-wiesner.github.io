...
&= \mathbb{E}_{p\left(Z\right)p\left(X\right)} \left[\sum_Y \frac{p\left(Y\right)e^{f\left(X, Y\right)}}{\mathbb{E}_{p\left(Y\right)} e^{f\left(X, Y\right)}} \left( f\left(X, Y\right) - f^{*} \right)\right] \\
\end{align}$$
