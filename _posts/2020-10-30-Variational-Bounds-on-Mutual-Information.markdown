...
&\implies \sum_Y p\left(Y | X_1\right) f\left(X_1, Y\right) \left(\mathbb{1}\left(Y_{\tau}, s\right) - \gamma_{X_1}\left(s, \tau\right) \right) \simeq \left(\hat{\gamma}_{X_1}\left(s, \tau\right) - \gamma_{X_1}\left(s, \tau \right)\right) \frac{\hat{Z}\left(X_1\right)}{Z\left(X_1\right)} \\
\end{align}$$
...
&\simeq \frac{1}{K} \left[ \hat{\gamma}_{X_j}\left(s, \tau\right) + \gamma_{X_j}\left(s, \tau \right) \left(1 - \frac{\hat{Z}\left(X_j\right)}{Z\left(X_j\right)}\right) + \sum_{k=1}^K \left(\gamma_{X_{k, k}}\left(s, \tau\right) - \gamma_{X_k}\left(s, \tau\right)\right) \frac{e^{E\left(k, k\right)}}{\sum_{i=1}^K e^{E\left(k, i\right)}} - \gamma_{X_{k, j}}\left(s, \tau\right) \frac{e^{E\left(k, j\right)}}{\sum_{i=1}^K e^{E\left(k, i\right)}}\right] \\
\end{align}$$
<!--
