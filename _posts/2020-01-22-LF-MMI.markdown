...
$$ \implies \nabla_{\theta} F_{MMI} = \sum_{r=1}^N \nabla_{\theta} \log{p_{\theta}\left(X_r | W_r \right)} - \nabla_{\theta} \log{\sum_{W} p_{\theta}\left(X_r | W \right) p\left(W \right)}$$
