...
* Words are modeled as a sequence of units. In traditional ASR these are triphones, but even just a sequence of letters would probably work fine. A single word could corresponding to different allowable sequences of units.
...
* For each of these units, there is an associated HMM. Traditionally this is a 3-state HMM, but for reasons I'll explain later, we tend to use a 2-state HMM instead.
* p(X_r | W_r)

## Gradient of MMI
I am going to assume that the underlying acoustic model is an HMM. This means that we are modeling $$p_{\theta}\left(X_r | W_r\right)$$ as the sum of 

$$ p_{\theta}\left(X_r | W_r\right) = \prod_{t=0}^{T
