...
\mbox{ (3)   } &=& E_{p\left(X_r, W_r\right)} \left[ - \log \left(\frac{p_{\theta}\left(X_r | W_r\right) p\left(W_r\right)}{\sum_{W} p_{\theta}\left(X_r | W\right) p\left(W\right)}\right)\right] 
\end{align}
...
$$ E_{p\left(X_r, W_r\right)} \left[ - \log \left(\frac{p_{\theta}\left(X_r | W_r\right) p\left(W_r\right)}{\sum_{W} p_{\theta}\left(X_r | W\right) p\left(W\right)}\right)\right]  = \lim_{N \to \infty} \frac{-1}{N} \sum_{r=1}^N\log\left(\frac{p_{\theta}\left(X_r | W_r\right) p\left(W_r\right)}{\sum_{W} p_{\theta}\left(X_r | W\right) p\left(W\right)}\right)$$

So finally, by approximating this limit by using a finite sample size $$N$$ ...

$$ E_{p\left(X_r, W_r\right)} \left[ - \log \left(\frac{p_{\theta}\left(X_r | W_r\right) p\left(W_r\right)}{\sum_{W} p_{\theta}\left(X_r | W\right) p\left(W\right)}\right)\right] \simeq \frac{-1}{N} \sum_{r=1}^{N}\log\left(\frac{p_{\theta}\left(X_r | W_r\right) p\left(W_r\right)}{\sum_{W} p_{\theta}\left(X_r | W\right) p\left(W\right)}\right)$$
...
## Gradient of MMI
One way to train the parameters of $$p_{\theta}\left(X_r | W_r\right)$$ is by gradient descent. In practice, rather than working directly with $$p_{\theta}\left(X_r | W_r\right)$$ it is easier to work with $$log \left(p_{\theta}\left(X_r | W_r\right)\right)$$.

The gradient can be computed as ...

$$\nabla_{\theta} F_{MMI} = \nabla_{\theta} \sum_{r=1}^N \log \left(\frac{p_{\theta}\left(X_r | W_r\right)p\left(W_r\right)}{\sum_{W} p_{\theta}\left(X_r | W\right)p\left(W\right)}\right),$$

$$ = \sum_{r=1}^N \nabla_{\theta} \log \left(\frac{p_{\theta}\left(X_r | W_r\right)p\left(W_r\right)}{\sum_{W} p_{\theta}\left(X_r | W\right)p\left(W\right)}\right)$$

by linearity of the gradient,

$$ = \sum_{r=1}^N \nabla_{\theta} \log \left(p_{\theta}\left(X_r | W_r\right)\right) + \nabla_{\theta} log \left(p\left(W_r\right)- \nabla_{\theta} log \left(\sum_{W} p_{\theta}\left(X_r | W\right)p\left(W\right)}\right)
$$
