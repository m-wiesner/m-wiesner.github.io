...
Our algorithm has some major problems however, especially the way we proposed to generate the numerator and denominator graphs. Below are details explaining how in practice we can generate these graphs.
...
Even this graph may be too big to use in training, especially if higher order n-gram models are used. The main contribution of LF-MMI is how it approximates this graph in order to make the computation feasible and the graph of a manageable size.
