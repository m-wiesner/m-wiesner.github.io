...
This means we can consider the gradient of $$\log{p_{\theta}\left(X_r, W \right)}$$ instead of $$\log{p_{\theta}\left(X_r | W \right)}$$. This happens to be very convenient when $$p_{\theta}\left(X_r, W \right)$$ is parameterized by an HMM.
