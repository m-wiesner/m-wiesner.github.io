...
&\implies \delta F_{MMI} = \sum_{r} \left[ \gamma_{r}\left(s, \tau\right)  - \gamma_{w^*}\left(s, \tau\right)\right] \delta y_s^{\tau}
\end{align}
...
$$ \delta F_{MMI} = \sum_{r} \left[ \gamma_{r}\left(s, \tau\right)  - \gamma_{w^*}\left(s, \tau\right)\right] \delta y_s^{\tau} $$

### FST implementation of the algorithm
Our algorithm has some major problems however. Most of these problems can be fixed using finite state transducers (FSTs). 
...
$$p\left(\lt s\rangle\gt | name \right) = \frac{c\left(\lt s\rangle\gt, name\right)}{c\left(name\right)} = 0.5$$
...
$$p\left(\lt s\rangle\gt | Matthew \right) = \frac{c\left(\lt s\rangle\gt, Matthew\right)}{c\left(Matthew\right)} = 0.5$$
...
![](/LF-MMI/graphviz (2).png){:height="50%" width="50%"}
