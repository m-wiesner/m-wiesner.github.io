...
## Gradient of $$\textbf{\log{p_{\theta}\left(X_r | W \right)}}$$
...
$$ = \frac{1}{\sum_{\pi_W} \prod_{t=0}^{T-1} p_{\theta}\left(x_t | s_t\right) p\left(s_t | s_{t-1}\right)} \cdot \sum_{\pi_W} \nabla_{\theta} \prod_{t=0}^{T-1} p_{\theta}\left(x_t | s_t\right) p\left(s_t | s_{t-1}\right) $$

We will again use our gradient of logarithms identity, but this time using $$x\left(\theta\right) = \prod_{t=0}^{T-1} p_{\theta}\left(x_t | s_t\right) p\left(s_t | s_{t-1}\right)$$.
