...
$$ \nabla_{\theta} \log{p_{\theta}\left(X_r, W\right)} = \sum_{t=0}^{T-1} \frac{\sum_{\pi_W} \nabla_{\theta} \log{p_{\theta}\left(x_t | s_t\right)} \prod_{\tau^{\prime}=0}^{T-1} p_{\theta}\left(x_{\tau^{\prime}} | s_{\tau^{\prime}}\right) p\left(s_{\tau^{\prime}} | s_{\tau^{\prime}-1}\right)}{\sum_{\pi_W} \prod_{\tau=0}^{T-1} p_{\theta}\left(x_{\tau} | s_{\tau}\right) p\left(s_{\tau} | s_{\tau-1}\right)} \tag{2}$$
...
Clearly summing over all sequences is not computationally feasible (exponential computational complexity). Instead we can use the computationally efficient forward and backward algorithms to compute $$\sum_{t=0}^{T-1} \sum_{\pi_W} p_{\theta}\left(X_r, \pi_W \right) \nabla_{\theta} \log{p_{\theta}\left(x_t | s_t\right)} $$ in the numerator of (2).
