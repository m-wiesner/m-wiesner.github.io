...
Equipped with this identity I proceed to take the gradient ...
...
In general, a particular word sequence can be pronounced in multiple ways. In Hybrid HMM-DNN ASR, we create 3-state HMM models for every possible triphone. In this way a word sequence can be decomposed into sequences of 3-state triphone HMMs which are strung together to form a model of the word sequence. The collection of these sequences forms a graph which we use to model a particular word sequence. The graph representing all possible word sequences clearly contains more many more paths, but it is still of the same form.
...
$$ \nabla_{\theta} \log{p_{\theta}\left(X_r, W\right)} = \frac{1}{\sum_{\pi_W} \prod_{t=0}^{T-1} p_{\theta}\left(x_t | s_t\right) p\left(s_t | s_{t-1}\right)} \sum_{t=0}^{T-1} \sum_{\pi_W} \nabla_{\theta} \log{p_{\theta}\left(x_t | s_t\right)} \prod_{\tau=0}^{T-1} p_{\theta}\left(x_\tau | s_\tau\right) p\left(s_\tau | s_{\tau-1}\right)$$

Finally pulling the denominator inside the first summation we have ...

$$ \nabla_{\theta} \log{p_{\theta}\left(X_r, W\right)} = \sum_{t=0}^{T-1} \frac{\sum_{\pi_W} \nabla_{\theta} \log{p_{\theta}\left(x_t | s_t\right)} \prod_{\tau^{\prime}=0}^{T-1} p_{\theta}\left(x_{\tau^{\prime}} | s_{\tau^{\prime}}\right) p\left(s_{\tau^{\prime}} | s_{\tau^{\prime}-1}\right)}{\sum_{\pi_W} \prod_{\tau=0}^{T-1} p_{\theta}\left(x_{\tau} | s_{\tau}\right) p\left(s_{\tau} | s_{\tau-1}\right)}$$

Next we note that 
$$ \prod_{\tau^{\prime}=0}^{T-1} p_{\theta}\left(x_{\tau^{\prime}} | s_{\tau^{\prime}}\right) p\left(s_{\tau^{\prime}} | s_{\tau^{\prime}-1}\right) = p_{\theta}\left(X_r, \pi_W \right)$$

and therefore $$ \sum_{\pi_W} p_{\theta}\left(X_r, \pi_W \right) = p_{\theta}\left(X_r \right) $$.

Substituting in these quantities we have ...

\begin{align}
\nabla_{\theta} \log{p_{\theta}\left(X_r, W\right)} &= \sum_{t=0}^{T-1} \sum_{\pi_W} p_{\theta}\left(\pi_W | X_r\right) \nabla_{\theta} \log{p_{\theta}\left(x_t | s_t\right)} \\\
&= \sum_{\pi_W} \sum_{t=0}^{T-1} p_{\theta}\left(\pi_W | X_r\right) \nabla_{\theta} \log{p_{\theta}\left(x_t | s_t\right)}
&= \sum_{s} \sum_{
\end{align}

Clearly summing over all sequences is not computationally feasible (exponential computational complexity). Instead we use the computationally efficient forward and backward algorithms to compute
